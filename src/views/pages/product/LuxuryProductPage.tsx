...
const LuxuryProductPage: NextPage<TProps> = () => {
  ...
  const handleLoadBrands = async () => {
    try {
      ...
      const items = response?.brands?.items || []
      setBrands(items)
    } catch (error) {
  ...
  const handleLoadCategories = async () => {
    try {
      ...
      const items = response?.categorys?.items || []
      setCategories(items)
    } catch (error) {
  ...
  const handleLoadMovementTypes = async () => {
    try {
      ...
      const items = response?.movementTypes?.items || []
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={9}>
            ...
            {loading ? (
              ...
              >
                <Typography sx={{ color: 'text.secondary' }}>Đang tải sản phẩm...</Typography>
              </Box>
