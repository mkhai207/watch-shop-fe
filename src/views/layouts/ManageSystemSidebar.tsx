      ...
      <Toolbar sx={{ display: 'flex', alignItems: 'center', justifyContent: open ? 'space-between' : 'center', px: 1 }}>
        {open && (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, ml: 1 }}>
            ...
            <Box sx={{ fontWeight: 700 }}>WatchStore</Box>
          </Box>
        )}
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            ...
            {flatItems.map(section => (
              <React.Fragment key={section.title}>
                {section.children?.map(child => (
                  <Tooltip key={child.title} title={open ? '' : child.title} placement='right' arrow>
                    <ListItemButton
        ...
        >
          {open ? (
            ...
            <Tooltip title='Về trang chủ' placement='right' arrow>
